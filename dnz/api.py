...
class Dnz():
    ...
    def search(self, q=None, **kwargs):
        """Build and performs an item search."""
...
class Request():
    ...
    def __init__(self, query=None, fields=None, facets=None, sort=None,
                 direction=None, per_page=None, page=None, facets_page=None,
                 facets_per_page=None, geo_bbox=None, _and=None, _or=None,
                 _without=None, key='', wild=None, quiet=True):
        ...
        if direction:
            url_parts.append(self._singleValueFormatter(
                'direction', direction))
    ...
    def _buildUrl(self, url_parts=None):
        """Create a URL with the URL parts."""
        url = [
            self.base_url,
            "?",
            "&".join(url_parts),
    ...
    def _handleGeoBox(self, geo_bbox):
        """Handle geometric box values."""
        if not len(geo_bbox) == 4:
            raise ValueError("geo_bbox should have exactly 4 values. e.g. \
                [-41,174,-42,175]")
            return ''
        geo_bbox = [
    ...
    def _singleValueFormatter(self, param_name, value):
        """Create an encoded URL fragment for parameters that contain only a single value."""
    ...
    def _multiDictFormatter(self, param_name, values):
        """Create an encoded URL fragment for parameters that are dictionaries."""
        url_parts = []
    ...
    def _searchFieldsFormatter(self, searchFields):
        """Create an encoded URL fragment.

        The fragment is used for searching for a value within a specific field.
...
class Results():
    ...
    def __repr__(self):
        """User friendly representation of a Results object."""
